(function (global, factory) {
  ...
  function handleMouseMove(e, config) {
    ...
    draggedEl.style.transform = "translate3d(".concat(config.axis === "both" || config.axis === "x" ? currentMousePosition.x - dragStartMousePosition.x : 0, "px, ").concat(config.axis === "both" || config.axis === "y" ? currentMousePosition.y - dragStartMousePosition.y : 0, "px, 0)");
  }
  function handleDrop() {
    printDebug(function () {
      return "dropped";
  ...
  function dndzone(node, options) {
    ...
    function handleDragStart() {
      ...
      mouseMoveListener = function mouseMoveListener(e) {
        return handleMouseMove(e, config);
      };
  ...
  var ID_TO_INSTRUCTION = _defineProperty(_defineProperty({}, INSTRUCTION_IDs.DND_ZONE_ACTIVE, "Tab to one the items and press space-bar or enter to start dragging it"), INSTRUCTION_IDs.DND_ZONE_DRAG_DISABLED, "This is a disabled drag and drop list");
  ...
  function alertCantMoveBecauseAxis(config, axis) {
    if (!config.autoAriaDisabled) {
      alertToScreenReader("Item ".concat(focusedItemLabel, " can not be moved on the ").concat(axis, " axis"));
    }
  }
  ...
  function dndzone$1(node, options) {
    ...
    function handleMoveItemDown(e) {
      ...
      if (idx > 0) {
        if (!config.autoAriaDisabled) {
          alertToScreenReader("Moved item ".concat(focusedItemLabel, " to position ").concat(idx, " in the list ").concat(focusedDzLabel));
        }
        swap(items, idx, idx - 1);
        dispatchFinalizeEvent(node, items, {
          trigger: TRIGGERS.DROPPED_INTO_ZONE,
  ...
  var _excluded = ["items", "flipDurationMs", "type", "dragDisabled", "morphDisabled", "dropFromOthersDisabled", "zoneTabIndex", "zoneItemTabIndex", "dropTargetStyle", "dropTargetClasses", "transformDraggedElement", "autoAriaDisabled", "centreDraggedOnCursor", "axis"];
